...
public class OpenFlowPacketParser {
	...
	//c		System.out.println((OFPacketOut) packetOuts.get(i).getMessage());
	...
	public boolean verifyFlowMods(ArrayList<Packet> flowMods) {
		OFMessage first = flowMods.get(0).getMessage();
	//	System.out.println(first);
		for (int i = 1; i < flowMods.size(); i++) {
		//	System.out.println(flowMods.get(i).getMessage());
			if (!first.equalsIgnoreXid(flowMods.get(i).getMessage())) {
				return false;
			}
		}
	...
	public int verifyPacketOutFailure(Packet packet, ArrayList<Packet> packetOuts) {
		...
		//	System.out.println(packetOuts.get(i).getMessage());
...
//
	...
	//	System.out.println(first);
		int count = 1;
		for (int i = 0; i < flowMods.size(); i++) {
			OFFlowMod compare = (OFFlowMod) flowMods.get(i).getMessage();
