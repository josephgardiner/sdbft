...
public class ServerConnection implements Runnable {
	...
	public ServerConnection(String iP, Socket s, BFTClientControl control, Controller controller, OpenFlowPacketParser parser, 	OFVersion ofVersion) {
		this.IP = iP;
		this.sock = s;
...
//						p.getMessage().writeTo(toWrite);
						System.out.println("Controller " + sock.getPort() + ": " + p.getType());
						// getAck
				...
				if (packets.size() > 0 && packets != null) { 
					...
					control.processResponseList(packets, controller.getId());
				}
//				
//				Thread.sleep(1);
