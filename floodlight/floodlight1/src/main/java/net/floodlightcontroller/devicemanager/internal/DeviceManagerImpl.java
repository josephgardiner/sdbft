...
public class DeviceManagerImpl implements IDeviceService, IOFMessageListener, ITopologyListener, IFloodlightModule, IEntityClassListener, IInfoProvider {
	...
	public int syncReceiveIntervalMs;
	 ...
	 */
	static final int DEFAULT_INITIAL_SYNC_STORE_CONSOLIDATE_MS = 15 * 1000; // 15 sec
	 ...
	 */
	static final int DEFAULT_SYNC_STORE_CONSOLIDATE_INTERVAL_MS = 75 * 60 * 1000; // 75 min
	...
	/**
	 * Device manager event listeners reclassifyDeviceListeners are notified first
	 * before reconcileDeviceListeners. This is to make sure devices are correctly
	 * reclassified before reconciliation.
	 */
	 ...
	 *
	 * Compares two attachment points and returns the latest one. It is assumed that
	 * the two attachment points are in the same L2 domain.
	 *
	 * @author srini
	 */
	protected class AttachmentPointComparator implements Comparator<AttachmentPoint> {
		...
		public int compare(AttachmentPoint oldAP, AttachmentPoint newAP) {
			...
			if (oldDomain.getLong() < newDomain.getLong())
				return -1;
			else if (oldDomain.getLong() > newDomain.getLong())
				return 1;

			// Give preference to LOCAL always
			if (oldPort != OFPort.LOCAL && newPort == OFPort.LOCAL) {
				return -1;
			} else if (oldPort == OFPort.LOCAL && newPort != OFPort.LOCAL) {
				return 1;
			}
			...
			// of the result.
			if (oldAP.getLastSeen().after(newAP.getLastSeen())) // TODO should this be lastSeen? @Ryan did change this
																// from activeSince
			...
			if ((newAP.getActiveSince().getTime() > oldAP.getLastSeen().getTime() + activeOffset) || (newAP
					.getLastSeen().getTime() > oldAP.getLastSeen().getTime() + AttachmentPoint.INACTIVITY_INTERVAL)) {
				return -1;
			}
			return 1;
		}
	...
	/**
	 * Periodic task to consolidate entries in the store. I.e., delete entries in
	 * the store that are not known to DeviceManager
	 */
	private SingletonTask storeConsolidateTask;

	private SingletonTask storeReceiveTask;
	...
	@Override
	public IDevice findDevice(@Nonnull MacAddress macAddress, VlanVid vlan, @Nonnull IPv4Address ipv4Address,
			@Nonnull IPv6Address ipv6Address, @Nonnull DatapathId switchDPID, @Nonnull OFPort switchPort)
			throws IllegalArgumentException {
		if (macAddress == null) {
			throw new IllegalArgumentException(
					"MAC address cannot be null. Try MacAddress.NONE if intention is 'no MAC'");
		}
		if (ipv4Address == null) {
			throw new IllegalArgumentException(
					"IPv4 address cannot be null. Try IPv4Address.NONE if intention is 'no IPv4'");
		}
		if (ipv6Address == null) {
			throw new IllegalArgumentException(
					"IPv6 address cannot be null. Try IPv6Address.NONE if intention is 'no IPv6'");
		}
		if (vlan == null) {
			throw new IllegalArgumentException(
					"VLAN cannot be null. Try VlanVid.ZERO if intention is 'no VLAN / untagged'");
		}
		if (switchDPID == null) {
			throw new IllegalArgumentException(
					"Switch DPID cannot be null. Try DatapathId.NONE if intention is 'no DPID'");
		}
		if (switchPort == null) {
			throw new IllegalArgumentException("Switch port cannot be null. Try OFPort.ZERO if intention is 'no port'");
		}

		Entity e = new Entity(macAddress, vlan, ipv4Address, ipv6Address, switchDPID, switchPort, Entity.NO_DATE);

		/*
		 * allKeyFieldsPresent() will check if the entity key fields (e.g. MAC and VLAN)
		 * have non-"zero" values i.e. are not set to e.g. MacAddress.NONE and
		 * VlanVid.ZERO
		 */
	...
	@Override
	public Iterator<? extends IDevice> queryDevices(@Nonnull MacAddress macAddress, VlanVid vlan,
			@Nonnull IPv4Address ipv4Address, @Nonnull IPv6Address ipv6Address, @Nonnull DatapathId switchDPID,
		...
		if (secondaryIndexMap.size() > 0) {
			EnumSet<DeviceField> keys = getEntityKeys(macAddress, vlan, ipv4Address, ipv6Address, switchDPID,
					switchPort);
		...
		if (index == null) {
			...
			// index lookup
			Entity entity = new Entity(macAddress, vlan, ipv4Address, ipv6Address, switchDPID, switchPort,
					Entity.NO_DATE);
			deviceIterator = new DeviceIndexInterator(this, index.queryByEntity(entity));
		}

		DeviceIterator di = new DeviceIterator(deviceIterator, null, macAddress, vlan, ipv4Address, ipv6Address,
	...
	public Iterator<? extends IDevice> queryClassDevices(@Nonnull IEntityClass entityClass,
			@Nonnull MacAddress macAddress, @Nonnull VlanVid vlan, @Nonnull IPv4Address ipv4Address,
			@Nonnull IPv6Address ipv6Address, @Nonnull DatapathId switchDPID, @Nonnull OFPort switchPort) {
		if (macAddress == null) {
			throw new IllegalArgumentException(
					"MAC address cannot be null. Try MacAddress.NONE if intention is 'no MAC'");
		...
		if (index == null) {
			...
			if (index == null) {
				...
				return new DeviceIterator(deviceMap.values().iterator(), new IEntityClass[] { entityClass }, macAddress,
						vlan, ipv4Address, ipv6Address, switchDPID, switchPort);
			} else {
	...
	protected Iterator<Device> getDeviceIteratorForQuery(@Nonnull MacAddress macAddress, VlanVid vlan,
			@Nonnull IPv4Address ipv4Address, @Nonnull IPv6Address ipv6Address, @Nonnull DatapathId switchDPID,
			@Nonnull OFPort switchPort) {
		if (macAddress == null) {
			throw new IllegalArgumentException(
	...
	public void startUp(FloodlightModuleContext fmc) throws FloodlightModuleException {
		...
		Map<String, String> configParams = fmc.getConfigParams(this);
		this.syncReceiveIntervalMs = Integer.parseInt(configParams.get("receiveIntervalMs"));
...
//		Runnable receiveStoreRunner = new Runnable() {
//			@Override
//			public void run() {
//				deviceSyncManager.updateSwitchDevices();
//				storeReceiveTask.reschedule(250, TimeUnit.MILLISECONDS);
//			}
//		};
//		storeReceiveTask = new SingletonTask(ses, receiveStoreRunner);
//
//		storeReceiveTask.reschedule(250, TimeUnit.MILLISECONDS);
	...
	private void registerDeviceManagerDebugCounters() throws FloodlightModuleException {
		...
		cntReconcileNoSource = debugCounters.registerCounter(PACKAGE, "reconcile-no-source-device",
				...
				IDebugCounterService.MetaData.WARN); // is this really a IDebugCounterService.CTR_MDATA_WARNing
		cntReconcileNoDest = debugCounters.registerCounter(PACKAGE, "reconcile-no-dest-device",
				"Number of flow reconcile events that failed because no " + "destination device could be identified",
				IDebugCounterService.MetaData.WARN); // is this really a IDebugCounterService.CTR_MDATA_WARNing
		cntInvalidSource = debugCounters
				.registerCounter(PACKAGE, "invalid-source",
		...
		cntDhcpClientNameSnooped = debugCounters.registerCounter(PACKAGE, "dhcp-client-name-snooped",
				"Number of times a DHCP client name was snooped from a " + "packetIn.");
		...
		cntPacketNotAllowed = debugCounters.registerCounter(PACKAGE, "packet-not-allowed",
				...
				IDebugCounterService.MetaData.WARN); // is this really a IDebugCounterService.CTR_MDATA_WARNing?
		cntNewDevice = debugCounters.registerCounter(PACKAGE, "new-device", "Number of times a new device was learned");
		...
		cntDeviceMoved = debugCounters.registerCounter(PACKAGE, "device-moved", "Number of times devices have moved");
		...
		cntEntityRemovedTimeout = debugCounters.registerCounter(PACKAGE, "entity-removed-timeout",
				"Number of times entities have been removed due to timeout " + "(entity has been inactive for "
						+ ENTITY_TIMEOUT / 1000 + "s)");
		...
		cntSyncException = debugCounters.registerCounter(PACKAGE, "sync-exception",
				...
				IDebugCounterService.MetaData.WARN); // it this an error?
		cntDevicesFromStore = debugCounters.registerCounter(PACKAGE, "devices-from-store",
		...
		cntConsolidateStoreDevicesRemoved = debugCounters.registerCounter(PACKAGE, "consolidate-store-devices-removed",
	...
	protected Command processPacketInMessage(IOFSwitch sw, OFPacketIn pi, FloodlightContext cntx) {
		...
		if (logger.isTraceEnabled()) {
			logger.trace("Received PI: {} on switch {}, port {} *** eth={}" + " *** srcDev={} *** dstDev={} *** ",
					new Object[] { pi, sw.getId().toString(), inPort, eth, srcDevice, dstDevice });
		}
	...
	/**
	 * Get sender IPv4 address from packet if the packet is an ARP packet and if the
	 * source MAC address matches the ARP packets sender MAC address.
	 * 
	 * @param eth
	 * @param dlAddr
	 * @return
	...
	protected Entity getSourceEntityFromPacket(Ethernet eth, DatapathId swdpid, OFPort port) {
		...
		return new Entity(dlAddr, vlan, ipv4Src, ipv6Src, swdpid, port, new Date());
	}
	 ...
	 */
	protected void learnDeviceFromArpResponseData(Ethernet eth, DatapathId swdpid, OFPort port) {
		...
		Entity e = new Entity(senderAddr, vlan, /* will either be a valid tag or VlanVid.ZERO if untagged */
				nwSrc, IPv6Address.NONE, /* must be none for ARP */
				swdpid, port, new Date());
	...
	protected Entity getDestEntityFromPacket(Ethernet eth) {
		...
		return new Entity(dlAddr, vlan, ipv4Dst, ipv6Dst, DatapathId.NONE, OFPort.ZERO, Entity.NO_DATE);
	}

	/**
	 * Look up a {@link Device} based on the provided {@link Entity}. We first check
	 * the primary index. If we do not find an entry there we classify the device
	 * into its IEntityClass and query the classIndex. This implies that all key
	 * field of the current IEntityClassifier must be present in the entity for the
	 * lookup to succeed!
	 * 
	...
	/**
	 * Get a destination device using entity fields that corresponds with the given
	 * source device. The source device is important since there could be ambiguity
	 * in the destination device without the attachment point information.
	 * 
	 * @param reference the source device's entity class. The returned destination
	 ...
	 *         findDeviceInClassByEntity(IEntityClass clazz, Entity entity) { // XXX
		...
		while (true) {
			...
			if (deviceKey != null) {
				...
				if (entity.hasSwitchPort()
						&& !topology.isAttachmentPointPort(entity.getSwitchDPID(), entity.getSwitchPort())) {
			...
			if (entity.hasSwitchPort()) {
				boolean moved = device.updateAttachmentPoint(entity.getSwitchDPID(), entity.getSwitchPort(),
						entity.getLastSeenTimestamp());
				if (moved) {
					// we count device moved events in sendDeviceMovedNotification()
					// TODO remove this. It's now done in the event handler as a result of the
					// update above... sendDeviceMovedNotification(device);
					if (logger.isTraceEnabled()) {
	 ...
	 * @return The {@link Device} object if found
	 */
	protected Device learnDeviceBySyncEntity(Entity entity) {
		ArrayList<Long> deleteQueue = null;
		LinkedList<DeviceUpdate> deviceUpdates = null;
		Device device = null;
	...
	protected EnumSet<DeviceField> findChangedFields(Device device, Entity newEntity) {
		EnumSet<DeviceField> changedFields = EnumSet.of(DeviceField.IPv4, DeviceField.IPv6, DeviceField.VLAN,
				DeviceField.SWITCH);
		 ...
		 * changedFields.remove(DeviceField.IPv4); if
		 * (newEntity.getIpv6Address().equals(IPv6Address.NONE))
		 * changedFields.remove(DeviceField.IPv6); /*if
		 * (newEntity.getVlan().equals(VlanVid.ZERO)) TODO VLAN is ZERO here, since the
		 * actual Device and Entity must have some sort of VLAN, either untagged (ZERO)
		 * or some value changedFields.remove(DeviceField.VLAN); if
		 * (newEntity.getSwitchDPID().equals(DatapathId.NONE) ||
		...
		for (Entity entity : device.getEntities()) {
			if (newEntity.getIpv4Address().equals(IPv4Address.NONE) || /* NONE means 'not in this packet' */
					entity.getIpv4Address()
							.equals(newEntity.getIpv4Address())) /*
																	 * these (below) might be defined and if they are
																	 * and changed, then the device has changed
																	 */
			...
			if (entity.getVlan()
					.equals(newEntity.getVlan())) /*
													 * these (below) must be defined in each and every packet-in, and if
													 * different signal a device field change
													 */
				changedFields.remove(DeviceField.VLAN);
			if (newEntity.getSwitchDPID().equals(DatapathId.NONE) || newEntity.getSwitchPort().equals(OFPort.ZERO)
	...
	protected boolean allKeyFieldsPresent(Entity e, EnumSet<DeviceField> keyFields) {
		for (DeviceField f : keyFields) {
			...
			case VLAN:
				if (e.vlan == null) { /*
										 * VLAN is null for 'don't care' or 'unspecified'. It's VlanVid.ZERO for
										 * untagged.
										 */
					return false; /*
									 * For key field of VLAN, the VLAN **MUST** be set to either ZERO or some value.
									 */
				}
				break;
			default:
	...
	/**
	 * Get the secondary index for a class. Will return null if the secondary index
	 * was created concurrently in another thread.
	 * 
	...
	/**
	 * method to delete a given device, remove all entities first and then finally
	...
	private EnumSet<DeviceField> getEntityKeys(@Nonnull MacAddress macAddress, VlanVid vlan, /*
																								 * A null VLAN means
																								 * 'don't care';
																								 * VlanVid.ZERO means
																								 * 'untagged'
		...
		if (vlan != null)
			keys.add(DeviceField.VLAN); /*
										 * TODO verify fix. null means 'don't care' and will conduct full search;
										 * VlanVid.ZERO means 'untagged' and only uses untagged index
										 */
		if (!ipv4Address.equals(IPv4Address.NONE))
			keys.add(DeviceField.IPv4);
		if (!ipv6Address.equals(IPv6Address.NONE))
			keys.add(DeviceField.IPv6);
	...
	// not used
	/*
	 * TODO then let's get rid of it? protected Device allocateDevice(Device device,
	 * Set <Entity> entities) { List <AttachmentPoint> newPossibleAPs = new
	 * ArrayList<AttachmentPoint>(); List <AttachmentPoint> newAPs = new
	 * ArrayList<AttachmentPoint>(); for (Entity entity : entities) { if
	 * (entity.switchDPID != null && entity.switchPort != null) { AttachmentPoint aP
	 * = new AttachmentPoint(entity.switchDPID, entity.switchPort, new Date(0));
	 * newPossibleAPs.add(aP); } } if (device.attachmentPoints != null) { for
	 * (AttachmentPoint oldAP : device.attachmentPoints) { if
	 * (newPossibleAPs.contains(oldAP)) { newAPs.add(oldAP); } } } if
	 * (newAPs.isEmpty()) newAPs = null; Device d = new Device(this,
	 * device.getDeviceKey(), device.getDHCPClientName(), newAPs, null, entities,
	 * device.getEntityClass()); d.updateAttachmentPoint(); return d; }
	 */
	...
	public void entityClassChanged(Set<String> entityClassNames) {
		/*
		 * iterate through the devices, reclassify the devices that belong to these
		 * entity class names
		 */
		Iterator<Device> diter = deviceMap.values().iterator();
		while (diter.hasNext()) {
	 ...
	 * create new device(s), remove entities from this device. If the device entity
	 * class did not change then it returns false else true.
	 * 
	 * @param device
	 */
	...
	/**
	 * For testing: sets the time between transition to MASTER and consolidate store
	 * 
	 * @param intervalMs
	 */
	...
	private class DeviceSyncManager {
		...
		/**
		 * Write the given device to storage if we are MASTER. Use this method if the
		 * device has significantly changed (e.g., new AP, new IP, entities removed).
		 * 
		...
		/**
		 * Write the given device to storage if we are MASTER and if the last write for
		 * the device was more than this.syncStoreIntervalNs time ago. Use this method
		 * to updated last active times in the store.
		 * 
		 * @param d the device to store
		 */
		...
		/**
		 * Remove the given device from the store. If only some entities have been
		 * removed the updated device should be written using
		...
		/**
		 * Remove the given Versioned device from the store. If the device was locally
		 * modified ignore the delete request.
		 * 
		...
		/**
		 * Synchronously transition from SLAVE to MASTER. By iterating through the store
		 * and learning all devices from the store
		 */
		...
		private void updateSwitchDevices() {
			IClosableIterator<Map.Entry<String, Versioned<DeviceSyncRepresentation>>> iter = null;
			try {
				iter = storeClient.entries();
			} catch (SyncException e) {
			...
			try {
				while (iter.hasNext()) {
					Versioned<DeviceSyncRepresentation> versionedDevice = iter.next().getValue();
					DeviceSyncRepresentation storedDevice = versionedDevice.getValue();
					logger.info("Received device from sync: " + storedDevice.toString());
					if (storedDevice == null)
						continue;
					cntDevicesFromStore.increment();
					for (SyncEntity se : storedDevice.getEntities()) {
						learnDeviceBySyncEntity(se.asEntity());
					}
				}
			} finally {
				if (iter != null)
			...
			storeReceiveTask.reschedule(250,

					TimeUnit.MILLISECONDS);
		}

		/**
		 * Actually perform the write of the device to the store FIXME: concurrent
		...
		/**
		 * Iterate through all entries in the sync store. For each device in the store
		 * check if any stored entity matches a live device. If no entities match a live
		 * device we remove the entry from the store.
		 *
		 * Note: we do not check if all devices known to device manager are in the
		 * store. We rely on regular packetIns for that. Note: it's possible that
		 * multiple entries in the store map to the same device. We don't check or
		 * handle this case.
		 *
		 * We need to perform this check after a SLAVE->MASTER transition to get rid of
		 * all entries the old master might have written to the store after we took
		 * over. We also run it regularly in MASTER state to ensure we don't have stale
					...
					for (SyncEntity se : storedDevice.getEntities()) {
						try {
							...
							IDevice d = findDevice(MacAddress.of(se.macAddress), VlanVid.ofVlan(se.vlan),
									IPv4Address.of(se.ipv4Address), IPv6Address.NONE, DatapathId.of(se.switchDPID),
	...
	/**
	 * For testing. Sets the syncService. Only call after init but before startUp.
	 * Used by MockDeviceManager
	 * 
	 * @param syncService
	 */
