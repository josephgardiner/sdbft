...
implements IFloodlightModule, ILearningSwitchService, IOFMessageListener, IControllerCompletionListener {
	...
	private void writeFlowMod(IOFSwitch sw, OFFlowModCommand command, OFBufferId bufferId,
			Match match, OFPort outPort, OFPacketIn pi) {
	...
	/**
	 * Writes a OFFlowMod to a switch. Does nto set xid from packetin
	 * @param sw The switch tow rite the flowmod to.
	 * @param command The FlowMod actions (add, delete, etc).
	 * @param bufferId The buffer ID if the switch has buffered the packet.
	 * @param match The OFMatch structure to write.
	...
	Ethernet eth = IFloodlightProviderService.bcStore.get(cntx, IFloodlightProviderService.CONTEXT_PI_PAYLOAD);
		...
		if (pkt instanceof IPv4) {
			...
			log.info("Packet found: " + ip_pkt.getSourceAddress().toString() + " -> "+ ip_pkt.getDestinationAddress().toString());
			
		}
              
		OFPort inPort = (pi.getVersion().compareTo(OFVersion.OF_12) < 0 ? pi.getInPort() : pi.getMatch().get(MatchField.IN_PORT));
		log.info("Learning switch processing packet in");
		...
		if (outPort == null) {
			...
			//     a lot of floods.
			log.info("learning switch flooding");
			...
			log.info("learning switch writing packet out and flow mod");
			this.pushPacket(sw, m, pi, outPort);
			this.writeFlowMod(sw, OFFlowModCommand.ADD, OFBufferId.NO_BUFFER, m, outPort, pi);
			if (LEARNING_SWITCH_REVERSE_FLOW) {
				...
				this.writeFlowMod(sw, OFFlowModCommand.ADD, OFBufferId.NO_BUFFER, mb.build(), inPort,pi);
			}
		}
