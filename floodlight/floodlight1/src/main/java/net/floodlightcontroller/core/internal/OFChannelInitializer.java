...
public class OFChannelInitializer extends ChannelInitializer<Channel> {
	...
	public OFChannelInitializer(IOFSwitchManager switchManager,
			...
			String keyStore, 
			String keyStorePassword, FloodlightModuleContext context) {
		super();
	...
	protected void initChannel(Channel ch) throws Exception {
		...
		Map<String, String> config = context.getConfigParams(OFSwitchManager.class);
		Boolean useSigs = Boolean.parseBoolean(config.get("use-signatures"));
		OFChannelHandler handler = new OFChannelHandler(
				...
				timer,
				ofBitmaps,
				defaultFactory, useSigs);
		
	
		log.info("Use signatures: " + useSigs );
		...
		if(useSigs){
			 int keyLength = Integer.parseInt(config.get("key-length"));
			 int sigLength = Integer.parseInt(config.get("sig-length"));
			 String keyAlgo = config.get("key-type");
			 String signAlgo = config.get("sign-algo");
			 
			 System.out.println(sigLength + " " + keyLength + " " + keyAlgo + " " + signAlgo);
				KeyPair keys = generateKeyPair(999, keyAlgo, keyLength);

		pipeline.addLast(PipelineHandler.OF_MESSAGE_DECODER,
				new OFMessageDecoderSign(keys, signAlgo, sigLength));
		
		//Generate keyPair
		pipeline.addLast(PipelineHandler.OF_MESSAGE_ENCODER,
				new OFMessageEncoderSign(keys, signAlgo, sigLength));
		}else{
			pipeline.addLast(PipelineHandler.OF_MESSAGE_DECODER,
	  ...
	  public static KeyPair generateKeyPair(long seed, String keyAlgo, int keyLength) throws Exception {
		    KeyPairGenerator keyGenerator = KeyPairGenerator.getInstance(keyAlgo);
